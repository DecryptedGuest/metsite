  ...
  function cadRefreshStatus() {
    return apiCall('/api/cad/status').then(function (s) {
      ...
      if (el) el.innerHTML =
        ...
        (s.voiceMode === 'worker' ? chip('Voice host: worker (HTTP)', '#9b6dff') : '') +
        (s.voiceMode === 'gateway' ? chip('Voice host: worker' + (s.worker && s.worker.connected ? ' (linked)' : ' (waiting)'), s.worker && s.worker.connected ? '#9b6dff' : '#f59e0b') : '') +
      ...
      if (note) {
        var v = s.voice || {};
        var externalMode = s.voiceMode === 'worker' || s.voiceMode === 'gateway';
        var wh = s.worker && s.worker.health;
        // In external modes the ElevenLabs key + voice connection live on the
