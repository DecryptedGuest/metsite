/* SCO-19 dashboard — parallel to the CID tryout dashboard, scoped to /api/sco19.
   ...
   Command review queue (approve/deny). */
(function () {
  const esc = window.escapeHtml || (s => String(s == null ? '' : s));
  let CTX = { canApprove: false, isDev: false };
  let liveTimer = null;
  let logFilter = 'PENDING';
  let sco19TryoutsById = {};   // id -> tryout summary, for the clickable detail view
  ...
  function wireNav() {
    ...
    document.querySelectorAll('#sco19-logfilter-tabs .filter-tab').forEach(tab => {
      tab.addEventListener('click', () => {
        document.querySelectorAll('#sco19-logfilter-tabs .filter-tab').forEach(t => t.classList.toggle('active', t === tab));
        logFilter = tab.dataset.filter; sco19LoadReviewLogs();
      });
    });
  }
  ...
  // ── Tryouts list ───────────────────────────────────────────────────
  window.sco19LoadTryouts = async function () {
    const tb = document.getElementById('sco19-tryouts-tbody');
    tb.innerHTML = '<tr><td colspan="6" class="table-loading"><div class="spinner"></div></td></tr>';
    try {
      const rows = await api('/api/sco19/tryouts');
      ...
      tb.innerHTML = rows.map(t => {
        ...
        const btns = [];
        if (canEnd && t.status === 'LIVE') btns.push(`<button class="btn btn-ghost btn-sm" ${stop}sco19CompleteTryout('${t.id}')"><i class="ti ti-check"></i> End</button>`);
        if (canEnd) btns.push(`<button class="btn btn-ghost btn-sm" ${stop}sco19CancelTryout('${t.id}')"><i class="ti ti-x"></i> Cancel</button>`);
        if (CTX.isDev) btns.push(`<button class="btn btn-ghost btn-sm" style="color:var(--red);" title="Delete (dev)" ${stop}sco19DeleteTryout('${t.id}')"><i class="ti ti-trash"></i></button>`);
        const actions = btns.length ? `<div style="display:flex;gap:6px;justify-content:flex-end;">${btns.join('')}</div>` : '';
        return `<tr style="cursor:pointer;" onclick="sco19OpenTryout('${t.id}')"><td>${fmtWhen(t.scheduledAt)}</td><td>${esc(t.hostName || '—')}</td><td>${esc(t.coHostName || 'N/A')}</td><td>${tryoutStatusBadge(t)}</td><td>${link}</td><td>${actions}</td></tr>`;
      }).join('');
  ...
  window.sco19CompleteTryout = async function (id) {
    try { await api('/api/sco19/tryouts/' + id + '/complete', { method: 'POST' }); showToast('Tryout ended', 'success'); sco19LoadTryouts(); }
    catch (e) { showToast(e.message, 'error'); }
  };
  ...
  window.sco19OpenTryout = function (id) {
    const t = sco19TryoutsById[id];
    if (!t) return;
    ...
    const canEnd = (t.isMine || CTX.isDev) && !['COMPLETED', 'CANCELLED'].includes(t.status);
    const btns = [`<button class="btn btn-ghost" onclick="closeModal('modal-sco19-log')">Close</button>`];
    if (canEnd && t.status === 'LIVE') btns.push(`<button class="btn btn-ghost" onclick="sco19CompleteTryout('${t.id}');closeModal('modal-sco19-log');"><i class="ti ti-check"></i> End</button>`);
    if (canEnd) btns.push(`<button class="btn btn-danger" onclick="sco19CancelTryout('${t.id}');closeModal('modal-sco19-log');"><i class="ti ti-x"></i> Cancel</button>`);
    if (CTX.isDev) btns.push(`<button class="btn btn-ghost" style="color:var(--red);" onclick="sco19DeleteTryout('${t.id}')"><i class="ti ti-trash"></i> Delete</button>`);
    document.getElementById('sco19-tlog-footer').innerHTML = btns.join('');
  };
  ...
  async function pollLive() {
    const wrap = document.getElementById('sco19-live-wrap');
    const statusEl = document.getElementById('sco19-live-status');
    const badge = document.getElementById('sco19-live-badge');
    try {
      const live = await api('/api/sco19/tryouts/live');
  ...
  function cmdBtn(tid, a, action, icon, label) {
    ...
    return `<button class="btn btn-ghost btn-sm" title="${label}" onclick="sco19Cmd('${tid}','${action}','${rid}','${un}')"><i class="ti ${icon}"></i></button>`;
  }
  window.sco19Cmd = async function (tid, action, rid, uname) {
    try {
      await api('/api/sco19/tryouts/' + tid + '/command', { method: 'POST', body: JSON.stringify({ action, targetRobloxId: rid || null, targetUsername: uname || null }) });
  ...
  // ── My tryout logs ─────────────────────────────────────────────────
  window.sco19LoadMyLogs = async function () {
    const tb = document.getElementById('sco19-mylogs-tbody');
    tb.innerHTML = '<tr><td colspan="7" class="table-loading"><div class="spinner"></div></td></tr>';
    try {
      const logs = await api('/api/sco19/tryout-logs/mine');
  ...
  // ── Review logs (Command) ──────────────────────────────────────────
  window.sco19LoadReviewLogs = async function () {
    const tb = document.getElementById('sco19-reviewlogs-tbody');
    if (!tb) return;
    tb.innerHTML = '<tr><td colspan="7" class="table-loading"><div class="spinner"></div></td></tr>';
    try {
      const logs = await api('/api/sco19/tryout-logs/pending?status=' + encodeURIComponent(logFilter));
      ...
      const badge = document.getElementById('sco19-review-badge');
  ...
  function logRow(l, review) {
    ...
    const del = CTX.isDev ? `<button class="btn btn-ghost btn-sm" style="color:var(--red);" title="Delete (dev)" onclick="event.stopPropagation();sco19DeleteLog('${l.id}',${review})"><i class="ti ti-trash"></i></button>` : '';
    return `<tr style="cursor:pointer;" onclick="sco19OpenLog('${l.id}',${review})"><td>${first}</td><td>${l.totalAttendees || 0}</td><td>${l.passedCount || 0}</td><td>${l.failedCount || 0}</td><td>${l.strikeCount || 0}</td><td>${logStatusBadge(l.status)}</td>
  ...
  // ── Log detail modal ───────────────────────────────────────────────
  window.sco19OpenLog = async function (id, review) {
    openModal('modal-sco19-log');
    const body = document.getElementById('sco19-tlog-body');
    const footer = document.getElementById('sco19-tlog-footer');
    body.innerHTML = '<div class="table-loading"><div class="spinner"></div></div>'; footer.innerHTML = '';
    try {
      const l = await api('/api/sco19/tryout-logs/' + id);
      document.getElementById('sco19-tlog-title').textContent = 'Tryout Log — ' + (l.hostName || '');
      ...
      const notesField = (!review && l.status === 'DRAFT')
        ? `<div class="form-group" style="margin-top:12px;"><label class="form-label">Notes for the reviewer</label><textarea class="form-control" id="sco19-log-notes" rows="2">${esc(l.notes || '')}</textarea></div>`
      ...
      const devDel = CTX.isDev ? `<button class="btn btn-ghost" style="color:var(--red);" onclick="sco19DeleteLog('${l.id}',${review})"><i class="ti ti-trash"></i> Delete</button>` : '';
      if (!review && l.status === 'DRAFT') {
        footer.innerHTML = `<button class="btn btn-ghost" onclick="closeModal('modal-sco19-log')">Close</button>${devDel}<button class="btn btn-primary" onclick="sco19SubmitLog('${l.id}')"><i class="ti ti-send"></i> Post for review</button>`;
      } else if (review && l.status === 'PENDING' && CTX.canApprove) {
        footer.innerHTML = `<input type="text" class="form-control" id="sco19-log-review-note" placeholder="Optional note / reason" style="flex:1;margin-right:8px;" />
          ${devDel}<button class="btn btn-danger" onclick="sco19ReviewLog('${l.id}','deny')"><i class="ti ti-x"></i> Deny</button>
          <button class="btn btn-primary" onclick="sco19ReviewLog('${l.id}','approve')"><i class="ti ti-check"></i> Approve</button>`;
      } else {
        footer.innerHTML = `<button class="btn btn-ghost" onclick="closeModal('modal-sco19-log')">Close</button>${devDel}`;
      }
    } catch (e) { body.innerHTML = `<div class="table-empty"><div class="table-empty-text">${esc(e.message)}</div></div>`; }
  };

  window.sco19SubmitLog = async function (id) {
    const notesEl = document.getElementById('sco19-log-notes');
    const notes = notesEl ? notesEl.value : undefined;
    try {
      await api('/api/sco19/tryout-logs/' + id + '/submit', { method: 'POST', body: JSON.stringify({ notes }) });
      closeModal('modal-sco19-log'); showToast('Posted for review', 'success'); sco19LoadMyLogs();
    } catch (e) { showToast(e.message, 'error'); }
  };
  window.sco19ReviewLog = async function (id, action) {
    const noteEl = document.getElementById('sco19-log-review-note');
    const note = noteEl ? noteEl.value : '';
    if (action === 'deny' && !note.trim()) return showToast('Add a reason to deny.', 'warning');
    try {
      await api('/api/sco19/tryout-logs/' + id + '/' + action, { method: 'POST', body: JSON.stringify({ note }) });
      closeModal('modal-sco19-log'); showToast(action === 'approve' ? 'Approved' : 'Denied', 'success'); sco19LoadReviewLogs();
    } catch (e) { showToast(e.message, 'error'); }
  };
  ...
  async function init() {
    wireNav();
    try {
      CTX = await api('/api/sco19/context');
      if (CTX.canApprove) {
